...
from data.utils import prepare_data_llm2clip
from model.pacl import llm2clip_pacl, llm2clip_pacl_rope, llm2clip_pacl_base, llm2clip_pacl_rope_base
...
l2v = LLM2Vec(llm_model, tokenizer, pooling_mode="mean", max_length=512, doc_max_length=512).to(device)


process = prepare_data_llm2clip(img_size=336)
...
def eval(model, dataset, root_dir, dataset_name):
    ...
    for d in tqdm(dataset):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            text_cls = l2v.encode(options, convert_to_tensor=True).to('cuda')
            image_features, text_features = model(input_pixels, text_cls)
...
def eval_4(model, dataset, root_dir, dataset_name):
    ...
    for d in tqdm(dataset):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            # Be sure to check which entries should be compared !!!!!
            text_cls = l2v.encode(options, convert_to_tensor=True).to(device)
            image_features, text_features = model(input_pixels, text_cls)
            text_probs = 100.0 * image_features @ text_features.T
...
def eval_MMVP(model, root_dir, dataset_name):
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            with torch.no_grad(), torch.cuda.amp.autocast():
                text_cls = l2v.encode([text1, text2], convert_to_tensor=True).to(device)
                # one image at a time!
                image_feature_1, text_features = model(img1, text_cls)
                text_probs_1 = 100.0 * image_feature_1 @ text_features.T   # 2 * 2
                image_feature_2, text_features = model(img2, text_cls)
    ...
    if "rope" in args.model_path:
        if "after" in args.model_path:
            ...
            if "base" in args.model_path:
                model = llm2clip_pacl_rope_base()
            else:
                model = llm2clip_pacl_rope()
    else:
        if "base" in args.model_path:
            model = llm2clip_pacl_base()
        else:
            model = llm2clip_pacl()
