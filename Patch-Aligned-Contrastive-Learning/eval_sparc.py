...
from model.pacl import sparc, sparc_rope
...
def eval(model, dataset, root_dir, dataset_name, local=False):
    ...
    for d in tqdm(dataset):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            ...
            logits_per_image = model.scoring(input_pixels, tokenized_text, local=local) # image_features @ text_features
            correct = 1 if logits_per_image[0][0] > logits_per_image[1][1] else 0
...
def eval_4(model, dataset, root_dir, dataset_name, local=False):
    prepositions = ['on', 'under', 'front', 'behind', 'left', 'right']
    opposite = {'on': 'under', 'under': 'on', 'front': 'behind', 'behind': 'front', 'left': 'right', 'right': 'left'}
    ...
    for d in tqdm(dataset):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            # Be sure to check which entries should be compared !!!!!
            logits_per_image = model.scoring(input_pixels, tokenized_text, local=local) # image_features @ text_features
            correct = 1 if logits_per_image[0][0] > logits_per_image[1][1]  and logits_per_image[0][0] > logits_per_image[2][2] and logits_per_image[0][0] > logits_per_image[3][3] else 0
...
def eval_COCO_VG(model, dataset, root_dir, dataset_name, local=False):
...
def eval_MMVP(model, root_dir, dataset_name, local=False):
    evaluate_mode = "t2i"
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            with torch.no_grad(), torch.cuda.amp.autocast():
                # one image at a time!
                text_probs_1 = model.scoring(img1, texts, local=local) # image_features @ text_features           
                text_probs_2 = model.scoring(img2, texts, local=local) # image_features @ text_features           
...
if __name__ == "__main__":
    ...
    parser.add_argument('--local', action='store_true', help='Flag to indicate if local similarity is used')
    ...
    if "rope" in args.model_path:
        model = sparc_rope(base_model="ViT-L-14-336")
    else:
        model = sparc(base_model="ViT-L-14-336")
    ...
    for name in saved_weights:
        if name[7:] not in model_weights:
            print("Warning: "+name+" in weights is not included\n")
    ...
    if args.dataset in ["mmvp", "mmvpvlm"]:
        eval_MMVP(model, args.root_dir, args.dataset, args.local)
    else:
        dataset = json.load(open(annotation_file))
        if args.dataset in ["a", "b"]: 
            eval(model, dataset, args.root_dir, args.dataset, args.local)
        elif args.dataset in ["a4", "b4"]: 
            eval_4(model, dataset, args.root_dir, args.dataset, args.local)
        elif args.dataset in ["cocoone", "cocotwo", "vgone", "vgtwo"]:
            eval_COCO_VG(model, dataset, args.root_dir, args.dataset, args.local)