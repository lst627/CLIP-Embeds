...
from model.pacl import SparcLoss, sparc, sparc_rope
import time
import numpy as np
...
def train_one_epoch(train_data_loader, model, optimizer, loss_fn, device, scheduler, num_of_steps):
    ...
    for i, (images, caps) in enumerate(train_data_loader):
        ...
        optimizer.zero_grad()
        #Forward
        visual_features, text_features, grouped_visual_features, mask = model(images, caps)
        #Calculating Loss
        _loss = loss_fn(visual_features, text_features, grouped_visual_features, mask)
...
def train_clip(batch_size, epochs):
    ...
    Model and Loss
    """
    model = sparc_rope(sigma=1.0/576, base_model="ViT-L-14-336")
    ...
    Train
    """
    # TODO: learned temperature?
    num_batches = math.ceil(len(train_dataset) // batch_size)
    num_of_steps = 0
    loss_fn = SparcLoss(temperature=0.1)
