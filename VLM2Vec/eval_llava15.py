...
# Check https://huggingface.co/lst627/LLaVA-1.5-7B-VLM2Vec-LoRA
...
question_template = '<image> Represent the given image with the following question: {}'
...
def eval(dataset, root_dir, dataset_name):
    ...
    for d in tqdm(dataset):
        ...
        if gold_prep[0] == "left" or gold_prep[0] == "right":
            prompt = f'Is the {object1} to the left of or to the right of the {object2}?' 
        elif gold_prep[0] == "on" or gold_prep[0] == "under":
            prompt = f'Is the {object1} at the bottom of the {object2} or at the top of the {object2}?' 
        else:
            prompt = f'Is the {object1} in the back of the {object2} or in the front of the {object2}?' 
        ...
        with torch.no_grad():
            ...
            inputs = processor(images=[Image.open(image_name)], text=question, return_tensors="pt")
            ...
            for option in options:
                ...
                cos_sim = model.compute_similarity(qry_output, tgt_output)
                text_probs.append(cos_sim.item())

        correct = 1 if text_probs[0] > text_probs[1] else 0
...
def eval_4(dataset, root_dir, dataset_name):
    ...
    for d in tqdm(dataset):
        ...
        #####
        prompt_a = f"Is the {object1} to the left of, to the right of, on, or under the {object2}?"
        prompt_b = f"Is the {object1} to the left of, to the right of, in front of, or behind the {object2}?"
        prompt = prompt_a if dataset_name == "a4" else prompt_b
        # The ground truth is always the first one
        ...
        correct = 1 if text_probs[0] > text_probs[1] and text_probs[0] > text_probs[2] and text_probs[0] > text_probs[3] else 0
...
def eval_COCO_VG(dataset, root_dir, dataset_name):
    ...
    for d in tqdm(dataset):
        if "coco" in annotation_file:
            image_name = os.path.join(root_dir, 'val2017/{}.jpg'.format(str(d[0]).zfill(12)))
        else:
            image_name = os.path.join(root_dir, 'vg_images/{}.jpg'.format(d[0]))
        ...
        part1 = caption[len("A photo of a "):]

        if "one" in annotation_file:
            part2 = caption[caption.index("on the "):]
        elif "coco" in annotation_file:
            if "above" in caption:
                part2 = caption[caption.index("above "):]
            elif "below" in caption:
                part2 = caption[caption.index("below "):]
            else:
                part2 = caption[caption.index("to the "):]
        else:
            part2 = caption[caption.index("to the "):]
        
        p = random.random()
        if p < 0.5:
            part2 = part2.replace(gold_prep[0], opposite[gold_prep[0]])
        else:
            part1 = part1.replace(gold_prep[0], opposite[gold_prep[0]])

        prompt = f"Is the {part1} or {part2}?"
...
def eval_MMVP(root_dir, dataset_name):
    evaluate_mode = "t2i"   # Different from What'sUp, we use text-to-image matching here
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            if dataset_name == "mmvpvlm":
                img1_name = os.path.join(image_dir, qtype1, f'{qid1}.jpg')
                img2_name = os.path.join(image_dir, qtype1, f'{qid2}.jpg')
            else:
                img1_name = os.path.join(image_dir, f'{qid1}.jpg')
                img2_name = os.path.join(image_dir, f'{qid2}.jpg')

            text1 = 'a photo of ' + statement1
            text2 = 'a photo of ' + statement2
            options = [text1, text2]
            prompt = questions[int(qid1)-1]

            with torch.no_grad():
                qry_output = []
                question = question_template.format(prompt)
                inputs = processor(images=[Image.open(img1_name)], text=question, return_tensors="pt")
                inputs = {key: value.to('cuda') for key, value in inputs.items()}
                qry_output.append(model(qry=inputs)["qry_reps"])
                inputs = processor(images=[Image.open(img2_name)], text=question, return_tensors="pt")
                inputs = {key: value.to('cuda') for key, value in inputs.items()}
                qry_output.append(model(qry=inputs)["qry_reps"])
                ...
                for i, option in enumerate(options):
                    ...
                    tgt_output = model(tgt=inputs)["tgt_reps"]
                    text_probs.append([])
                    for qry in qry_output:
                        cos_sim = model.compute_similarity(qry, tgt_output)
                        text_probs[i].append(cos_sim.item())
...
if __name__ == "__main__":
    ...
    with open("evaluation_results.txt", "a") as f:
        f.write("Model path: llava-v1.5-7b ")
        f.write("Dataset: {}\n".format(args.dataset))
    
    if args.dataset in ["mmvp", "mmvpvlm"]:
        eval_MMVP(args.root_dir, args.dataset)
    else:
