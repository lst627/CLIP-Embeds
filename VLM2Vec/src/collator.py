...
class EvalCollator:
    ...
    def _get_batch_inputs(self, examples):
        ...
        for example in examples:
            text, image = example
            if image is None:
                if self.model_args.model_backbone == "llava_next" or self.model_args.model_backbone == "llava-hf/llava-1.5-7b-hf":
                    inputs = self.processor(images=None, text=text, return_tensors="pt")
                else:
                ...
                input_ids.append(inputs["input_ids"].squeeze(0).unsqueeze(1))
                pixel_values.append(inputs['pixel_values'])
                if 'image_sizes' in inputs: image_sizes.append(inputs['image_sizes'])
                else: image_sizes.append(None)
        ...
        if not image_exist:
            ...
            pixel_values = torch.cat(pixel_values, dim=0)
            if self.model_args.model_backbone == "llava-hf/llava-1.5-7b-hf":
                inputs = {
                    'input_ids': input_ids,
                    'attention_mask': attention_mask,
